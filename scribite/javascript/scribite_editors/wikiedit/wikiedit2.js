...
// if (!this.area.id) this.area.id = "area_"+String(Math.floor(Math.random()*10000));
 ...
 this.addButton(" ");
//seems to work incorrect this.addButton("indent","Indent","'  ','',0,1");
//seems to work incorrect this.addButton("outdent","Outdent","","document.getElementById('" + this.id + "')._owner.unindent");
// this.addButton(" ");
...
WikiEdit.prototype.createImage = function (isAlt)
{
  ...
  if (!n.test(this.sel)) {
    if (!isAlt) {
     lnk = prompt("Image URL:", this.sel);
     if (lnk==null) lnk = this.sel;
     sl = prompt("Title for image:", this.sel);
     if (lnk==null) lnk = this.sel;
     align = prompt("Align for image (center, right or left):", this.sel);
     if (align==null) align = "";
     this.sel = '{{image url="'+lnk+'" title="'+sl+'" alt="'+sl+'" class="'+align+'"}}';
    };
	str = this.sel1+this.trim(this.sel)+this.sel2;
    t.value = str;
    t.setSelectionRange(this.sel1.length, str.length-this.sel2.length);
