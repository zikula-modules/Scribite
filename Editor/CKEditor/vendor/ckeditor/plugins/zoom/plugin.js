 ...
 * Copyright (C) 2008-2013 Alfonso Martнnez de Lizarrondo
...
CKEDITOR.plugins.add( 'zoom',
{
	...
	init : function( editor )
	{
		...
		var content = '.cke_combopanel__zoom { height: 200px; width: 100px; }' +
					'.cke_combo__zoom .cke_combo_text { width: 42px;}'; // width: 40px
		...
		editor.ui.addRichCombo( 'Zoom',
			{
				label : editor.config.language == 'bg' ? 'Мащаб' : 'Zoom',
				title : editor.config.language == 'bg' ? 'Мащабиране полето на редактора' : 'Zoom editor box',
                toolbar: 'tools', // styles?
				multiSelect : false,
				className : 'zoom',
				...
				init : function()
				{
					var zoomOptions = [50, 75, 85, 100, 115, 125, 150, 200, 400],
						zoom;

					this.startGroup( editor.config.language == 'bg' ? 'Ниво увеличение' : 'Zoom level' );
