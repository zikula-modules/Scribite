...
( function() {
	var imageDialog = function( editor, dialogType ) {
            ...
            // convert image data in url to base64 string
            var setAttributeSrcAsBase64 = function (element, url) {
                var xhr = new XMLHttpRequest();
                xhr.onload = function() {
                    var reader = new FileReader();
                    reader.onloadend = function() {
                        element.data( 'cke-saved-src', reader.result );
			...
			return {
				...
				onOk: function() {
					...
					if ( this.imageEditMode ) {
						...
						if ( dialogType == 'image' && imgTagName == 'input' && confirm( editor.lang.imageplus.button2Img ) ) { // jshint ignore:line
							// Replace INPUT-> IMG
							imgTagName = 'img';
						...
						else if ( dialogType != 'image' && imgTagName == 'img' && confirm( editor.lang.imageplus.img2Button ) ) { // jshint ignore:line
							// Replace IMG -> INPUT
							imgTagName = 'input';
				...
				contents: [ {
					id: 'info',
					label: editor.lang.imageplus.infoTab,
					accessKey: 'I',
					elements: [ {
						type: 'vbox',
						padding: 0,
						children: [ {
							...
							children: [ {
								...
								onChange: function() {
                                    ...
                                    // activate base64 checkbox, if no reason to disable
                                    if (CKEDITOR.config.imageplus.base64option && newUrl.indexOf("data:") !== 0 ) {
                                        dialog.getContentElement( 'info', 'base64src' ).setValue(false);
                                        dialog.getContentElement( 'info', 'base64src' ).getInputElement().removeAttribute('disabled');
                                    }
								},
								...
								commit: function( type, element ) {
									if ( type == IMAGE && ( this.getValue() || this.isChanged() ) ) {
                                        ...
                                        var srcorigin = this.getValue();
                                        var isBase64url =  srcorigin.indexOf("data:") === 0;
                                        if (!isBase64url && dialog.getValueOf( 'info', 'base64src' )) {
                                            // set src attribute as base64 string
                                            var result = setAttributeSrcAsBase64(element, srcorigin);
                                            // set data-srcorigin attribute to point to original image
                                            element.data( 'cke-origin-src', srcorigin );
                                        } else {
								...
								validate: CKEDITOR.dialog.validate.notEmpty( editor.lang.imageplus.urlMissing )
							},
							{
								type: 'button',
								id: 'browse',
						...
						},
                        {
							...
							className: 'cke_dialog_image_base64',
                            hidden: !CKEDITOR.config.imageplus.base64option,
							children: [ {
                                id: 'base64src',
                                type: 'checkbox',
                                label: editor.lang.imageplus.base64src,
                                setup: function( type, element ) {
                                    if ( type == IMAGE ) {
                                        var value;
										var isBase64url =  element.getAttribute('src').indexOf("data:") === 0;
                                        if (isBase64url) {
                                            // save data for onChange
                                            this.cke_saved_src = element.data( 'cke-saved-src' );
                                            this.cke_origin_src = element.data( 'cke-origin-src' );
                                            // set value
                                            this.setValue( true );
                                        }
                                    }
                                ...
                                onChange: function() { 
                                        var dialog = this.getDialog();
                                        var origin_src = this.cke_origin_src;
                                        if (origin_src && !this.getValue()) {
                                            txt_url = dialog.getValueOf( 'info', 'txtUrl');
                                            var isBase64url = !txt_url || txt_url.indexOf("data:") === 0;
                                            if (isBase64url) {
                                                this.cke_origin_src = '';
                                                dialog.setValueOf( 'info', 'txtUrl', origin_src );
                                            }
                                        }
                                    }
							},
							{
								...
								align: 'right',
                                hidden: !CKEDITOR.config.imageplus.base64_localfile,
								label: editor.lang.imageplus.browseLocal,
                                onChange: function(){ 
                                    // this = this.getDialog().getContentElement( 'info', 'base64src_browse' )
                                    var dialog = this.getDialog();
                                    if (this.getValue()) {
                                        // there is file chosen
                                        var file = this.getInputElement().$.files[0];
                                        var reader = new FileReader();
                                        reader.onloadend = function() {
                                            dialog.setValueOf( 'info', 'txtUrl', reader.result );
                                            dialog.getContentElement( 'info', 'base64src' ).setValue(true);
                                            dialog.getContentElement( 'info', 'base64src' ).getInputElement().setAttribute('disabled', 'disabled');
                                        }
							...
							children: [ {
								...
								children: [ {
                                    ...
                                    commit: function( type, element ) {
                                        var value = this.getValue();
                                        if ( type == IMAGE ) {
                                            // imageplus: Change from style to attribute, to compatible with image2
                                ...
                                },
								{
									...
									style: 'margin-top: 25px !important;width:40px;',
									onLoad: function() {
									...
									html: '<div>' +
										'<a href="javascript:void(0)" tabindex="-1" title="' + editor.lang.imageplus.lockRatio +
										'" class="cke_btn_locked" id="' + btnLockSizesId + '" role="checkbox"><span class="cke_icon"></span><span class="cke_label">' + editor.lang.imageplus.lockRatio + '</span></a>' +
										'<a href="javascript:void(0)" tabindex="-1" title="' + editor.lang.imageplus.resetSize +
										'" class="cke_btn_reset" id="' + btnResetSizeId + '" role="button"><span class="cke_label">' + editor.lang.imageplus.resetSize + '</span></a>' +
										'</div>'
								} ]
							},
							{
								...
								children: [ {
									...
									width: '60px',
									label: editor.lang.imageplus.border,
									'default': '',
									onKeyUp: function() {
										updatePreview( this.getDialog() );
									},
									...
									validate: CKEDITOR.dialog.validate.integer( editor.lang.imageplus.validateBorder ),
									setup: function( type, element ) {
										if ( type == IMAGE ) {
											var value,
                                    ...
                                    children: [ 
                                        {
                                            ...
                                            label: editor.lang.imageplus.hSpace,
                                            title: editor.lang.common.left,
                                            'default': '',
                                            onKeyUp: function() {
                                                updatePreview( this.getDialog() );
                                            },
                                            ...
                                            validate: CKEDITOR.dialog.validate.integer( editor.lang.imageplus.validateHSpace ),
                                            setup: function( type, element ) {
                                                if ( type == IMAGE ) {
                                        ...
                                        },
                                        {
                                            ...
                                            title: editor.lang.common.right,
                                            width: '60px',
                                            'default': '',
                                            onKeyUp: function() {
                                                updatePreview( this.getDialog() );
                                    ...
                                    children: [
                                        {
                                            ...
                                            width: '60px',
                                            style: 'max-width:70px;',
                                            label: editor.lang.imageplus.vSpace,
                                            title: editor.lang.common.alignTop,
                                            'default': '',
                                            onKeyUp: function() {
                                                updatePreview( this.getDialog() );
                                            },
                                            ...
                                            validate: CKEDITOR.dialog.validate.integer( editor.lang.imageplus.validateVSpace ),
                                            setup: function( type, element ) {
                                                if ( type == IMAGE ) {
                                        ...
                                        },
                                        {
                                            ...
                                            width: '60px',
                                            title: editor.lang.common.alignBottom,
                                            'default': '',
                                            onKeyUp: function() {
                                                updatePreview( this.getDialog() );
                                            },
								...
								},
								{
									...
									'default': '',
									items: [
										[ editor.lang.common.alignNone, '' ],
										[ editor.lang.common.left, 'left' ],
										[ editor.lang.common.right, 'right' ]
										// Backward compatible with v2 on setup when specified as attribute value,
										// while these values are no more available as select options.
										//	[ editor.lang.imageplus.alignAbsBottom , 'absBottom'],
										//	[ editor.lang.imageplus.alignAbsMiddle , 'absMiddle'],
										//  [ editor.lang.imageplus.alignBaseline , 'baseline'],
										//  [ editor.lang.imageplus.alignTextTop , 'text-top'],
										//  [ editor.lang.imageplus.alignBottom , 'bottom'],
										//  [ editor.lang.imageplus.alignMiddle , 'middle'],
										//  [ editor.lang.imageplus.alignTop , 'top']
									],
									onChange: function() {
										updatePreview( this.getDialog() );
										commitInternally.call( this, 'advanced:txtdlgGenStyle' );
						...
						},
						{
							...
							children: [ {
								...
								html: '<div>' + CKEDITOR.tools.htmlEncode( editor.lang.common.preview ) + '<br>' +
									...
									'<div class="ImagePreviewBox" style="width:auto;min-height:220px;max-width:600px;max-height:350px;height:auto;"><table><tr><td>' +
									...
									// jscs:disable maximumLineLength
										( editor.config.image_previewText || 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit. ' +
											...
											'nulla. Aenean dictum lacinia tortor. Nunc iaculis, nibh non iaculis aliquam, orci felis euismod neque, sed ornare massa mauris sed velit. Nulla pretium mi et risus. Fusce mi pede, tempor id, cursus ac, ullamcorper nec, enim. Sed tortor. Curabitur molestie. Duis velit augue, condimentum at, ultrices a, luctus ut, orci. Donec pellentesque egestas eros. Integer cursus, augue in cursus faucibus, eros pede bibendum sem, in tempus tellus justo quis ligula. Etiam eget tortor. Vestibulum rutrum, est ut placerat elementum, lectus nisl aliquam velit, tempor aliquam eros nunc nonummy metus. In eros metus, gravida a, gravida sed, lobortis id, turpis. Ut ultrices, ipsum at venenatis fringilla, sem nulla lacinia tellus, eget aliquet.' ) +
				...
				},
				{
					...
					label: editor.lang.imageplus.linkTab,
					padding: 0,
					elements: [ {
						id: 'txtUrl',
						type: 'text',
					...
					},
					{
						...
						style: 'float:right',
						label: editor.lang.imageplus.linkCopyImageUrl,
						onClick: function() {
							var dialog = this.getDialog();
                            var value = dialog.getValueOf( 'info', 'txtUrl' );
                            dialog.setValueOf( 'Link', 'txtUrl', value );
                            if (!dialog.getValueOf( 'Link', 'txtRel' )) {
                                dialog.setValueOf( 'Link', 'txtRel', CKEDITOR.config.imageplus.a_copy_attr_rel ? CKEDITOR.config.imageplus.a_copy_attr_rel : '' );
                            }
						}
					},
					{
				...
				},
				{
					...
					label: editor.lang.imageplus.upload,
					elements: [ {
						type: 'file',
						id: 'upload',
						label: editor.lang.imageplus.btnUpload,
						style: 'height:40px',
						size: 38,
                        onChange: function() {
                                ...
                                var imgPrevSize = document.getElementById('imgPrevSize');
                                if (files && files[0]) {
                                    var reader = new FileReader();
                                    reader.onloadend = function () {
                                        var image = new Image();
                                        image.onload = function() {
                                                var size_b = files[0].size;
                                                var size_kb = Math.round(size_b / 1024);
                                                imgPrevSize.textContent = size_b.toLocaleString() + ' B (' + size_kb.toLocaleString() + ' kB), ' + this.width + ' x ' + this.height + ' px';
                                            };
                                        image.src = reader.result;
                                        imgPreview.setAttribute('src', reader.result);
                                        var size_b = files[0].size;
                                        var size_kb = Math.round(size_b / 1024);
                                        imgPrevSize.textContent = size_b.toLocaleString() + ' B (' + size_kb.toLocaleString() + ' kB)';
                                    }
                                    reader.readAsDataURL(files[0]);
                                } else {
                                    imgPreview.setAttribute('src', '');
                                    imgPrevSize.textContent = '';
                                }
                            }
					},
					{
					...
					},
					{
						type: 'html',
						id: 'htmlPreview',
						html: '<span id="imgPrevSize"></span><br /><img id="imgPreview" src: "" alt="" style="max-width: 500px; max-height: 500px;" />'
					} ]
				},
				{
					id: 'advanced',
					...
					},
					{
						...
						},
						{
                            id: 'txtAlt',
                            type: 'text',
                            label: editor.lang.imageplus.alt,
                            'default': '',
                            onChange: function() {
                                updatePreview( this.getDialog() );
                            },
	...
	CKEDITOR.dialog.add( 'imageplus', function( editor ) {
